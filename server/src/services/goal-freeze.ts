...
/**
 * 目標の一時凍結（spec: goal-freeze MODIFIED・issue #103）。
 *
 * 種別（当日凍結／期間凍結）と予約フェーズを廃止し、常に**当日発効**・終了日は自由指定の
 * 単一凍結へ統合した（design D1）。凍結の状態（凍結中/解凍済み）は保存せず、
 * `goal_freeze.start_day`/`end_day` と対象日から都度導出する。日次 cron を使わない
 * 既存方針（`goal-check-gate`）に揃える。
 *
 * `goal_freeze` は生きている凍結（解除は `end_day` の切り詰め）を持ち、
 * `goal_freeze_change` は起きた事実の追記専用ログ（`goal_freeze` へ FK を張らない）。
 * 月枠はログではなく `goal_freeze` の行の存在（`start_day` の `YYYY-MM`）から導出する。
 */

export type FreezeState = 'frozen' | 'released';
...
/** 月枠の判定基準は**予約日（＝当日）の月**（design D1）。常に発効日＝today と同じ月になる。 */
...
/** 凍結区間の状態（凍結中・解凍済み）を対象日から導出する（design D1）。 */
...
/**
 * 経過した凍結日数の合計（各区間 `[start, min(end, today)]`・未到来分は数えない）。
 */
...
/** 実効 end_day（凍結日数ぶん前方向へ延長・design D1）。 */
 ...
 * 再開が発効済み（`today >= resumed_day_key`）なら、ゲートには戻す。
 */
export function endedGoalIdsOn(db: DB, dayKey: string): Set<number> {
  const rows = db
    .prepare(
      `SELECT id FROM goal
        WHERE ended_day_key IS NOT NULL AND ended_day_key <= ?
          AND (resumed_day_key IS NULL OR resumed_day_key > ?)`,
    )
    .all(dayKey, dayKey) as { id: number }[];
...
/** その月に `start_day` を持つ凍結行（アプリ全体で1つ・design D1）。無ければ null。 */
...
/** その目標の最新の凍結（凍結中/解凍済み）。一度も凍結したことが無ければ null。 */
...
/** 凍結する（常に当日発効・理由必須・終了日は当日以降で自由指定・design D1）。 */
export function freezeGoal(
  db: DB,
  goalId: number,
  input: { endDay: string; reason: string },
  nowMs = Date.now(),
): FreezeView {
  return freezeGoalMulti(db, [goalId], input, nowMs)[0]!;
}

/** 複数の目標をまとめて凍結する（1回の月枠機会で複数選択・design D1）。 */
export function freezeGoalMulti(
  db: DB,
  goalIds: number[],
  input: { endDay: string; reason: string },
  nowMs = Date.now(),
