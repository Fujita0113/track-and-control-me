...
// --- ドラッグ確定(離席記録)ポップオーバー -------------------------------
// カテゴリは1欄に統合: 入力した語 = カテゴリ = ブロック名 = レジストリ登録。メモ欄は廃止。
function openDraft(startMin, endMin, x, y) {
  const cats = chipCategories();
  // st.category = 選択中のチップ名。新規入力(catInp)が非空ならそちらを優先する。
  const st = { start: startMin, end: endMin, category: cats[0] || DEFAULT_CATEGORIES[0] };
  ...
  // カテゴリ = 直近使用順チップ or 新規入力。入力語がそのままブロック名になり、レジストリへ登録される。
  ...
  // チップは「新規カテゴリ入力が空」かつ「選択中カテゴリと一致」のとき active。
  const syncChips = () => {
    const typing = !!catInp.value.trim();
    for (const { c, chip } of chips) {
      chip.classList.toggle('active', !typing && st.category === c);
    }
  };
  chips.forEach(({ chip }) => chipHost.appendChild(chip));
  syncChips();
  catInp.addEventListener('input', syncChips);
  ...
  const submit = async () => {
    ...
    // カテゴリ = 新規入力があればそれ、なければ選択チップ。ブロック名も同じ語。
    const category = catInp.value.trim() || st.category;
    if (!category) { toast('カテゴリを選ぶか入力してください', 'err'); return; }
    addBtn.disabled = true;
    try {
      await api.addManual(ctx.date, { startAt, endAt, title: category, color: 'grey', category });
      toast('離席を記録しました', 'ok');
      closePopover();
  ...
  // Enter で確定(新規カテゴリ名欄から)。IME 変換確定の Enter は isComposing で無視する。
  ...
  const node = h('div', {},
    ...
    h('div', { class: 'tlc-draft-lbl', text: 'カテゴリ(直近使用順)' }),
    chipHost,
