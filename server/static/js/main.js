...
async function boot() {
  ...
  try {
    ...
    // その日はじめてダッシュボードを開いたとき、回答すべき写真/質問ルールがあれば1回だけトースト（D7）。
    // デモは別経路（demo:refresh）で扱う。時刻でスケジュールはしない（croner・OS トースト不使用）。
    if (!isDemo()) await maybeShowDueRuleToast(state.today);
  } catch (err) {
