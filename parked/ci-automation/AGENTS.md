# track-and-control-me — エージェント向けプロジェクトルール

> このファイルは opencode 用のプロジェクトルールです。
> Claude Code 用の `CLAUDE.md` と**同じ内容を保つこと**（片方だけ更新しない）。

## プロジェクト概要

Edge のタブグループ単位で作業時間を計測し、パスワードゲートをかけるローカル完結アプリ。

- `extension/` — Edge 拡張（MV3）。WebSocket で server にハートビートを送る
- `server/` — Fastify + better-sqlite3。集計・ルール評価・ダッシュボード配信（`server/static/`）
- `packages/contract/` — 拡張と server が共有する zod スキーマ
- `openspec/` — 仕様の正典。`specs/` がメインスペック、`changes/` が進行中の変更
...
### 無人実行（CI: `xs-auto.yml`）には投げ返し先が無い

`size/XS` の自動実装には聞く相手がいない。そこでの答えは**常に「変更するな」**とする。
凍結側はそのままに、ユーザーへ見せるはずだった根拠を `tasks.md` の末尾へ追記し、
**赤いまま残して終える**（人間がレビューする赤い PR のほうが、
モデルが自分で書き換えて緑にした PR より安全）。
...
`openspec/specs/<capability>/spec.md` はこのプロジェクトの正典であり、以降の全変更の土台になる。
`/opsx-sync` でここを書き換えるときは：

- **既存の日本語の言い回し・粒度・見出し構造を保つ**。勝手に要約したり英語化したりしない
- delta spec に書かれていない要件・シナリオは**触らない**
- ADDED / MODIFIED / REMOVED / RENAMED の指示どおりに、最小限の差分だけを当てる
...
npm run test:e2e      # Playwright（デモモードで実ブラウザ検証）
```

E2E は `DB_PATH=:memory:` でサーバを自動起動するので、本番 DB には触らない。

## CI 自動実装フロー（`.github/workflows/xs-auto.yml`）

タスクサイズ `size/XS` の issue から自動で PR を作る際の約束事：

- **git のコミット・push・PR 作成はエージェントが行わない**。ファイル編集だけを行い、
  コミット以降はワークフローのシェルステップが担当する
- `/opsx-archive` は PR 内では実行しない。マージ後に別ワークフローが行う
