...
ルール編集 UI は条件を単一の `<select>` で選択 SHALL する。`PLANNING` ターゲットは「就寝前リチュアル」等の抽象ラベル＋別セレクト（「シグナル」）に束ねず、`signal_key` ごとの独立した項目（`今日の振り返り` / `明日のタスク登録` / `翌日計画（振り返り＋明日タスク）`）として、他ターゲット（総作業時間 / グループ作業 / 手動チェック）と同列に日本語ラベルで提示 SHALL する。凍結済みルールが未知の `signal_key` を持つ場合はその値を選択肢に温存し破壊的に上書きしない。ゲート画面・条件テキストは同じ日本語ラベルを接頭辞なしで表示する。サーバーのデータモデル（`target=PLANNING` ＋ `signal_key`）は変更しない（純粋に表示上の統合）。

#### Scenario: 条件ドロップダウンに振り返り/カンバンが直接並ぶ

- **WHEN** 未来ルール編集で条件のドロップダウンを開く
- **THEN** `今日の振り返り`・`明日のタスク登録` が他ターゲットと同列の選択肢として現れ、「シグナル」用の2つ目のセレクトは存在しない
