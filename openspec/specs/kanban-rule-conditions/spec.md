...
解錠ゲートの `PLANNING` 条件を、自己申告ではなく実データ由来のシグナル（`reflection_done` / `tomorrow_tasks_registered` / `tomorrow_planned`）で評価する。カンバンのタスク期日を列と「明日トグル」から自動決定（例外は手動ロック）し、翌日タスクの検出を確実にする。ルール編集 UI は条件を optgroup でカテゴリ化した単一ドロップダウンで選べるようにする。サーバーのデータモデル（`target=PLANNING` ＋ `signal_key`）と既存 `signal_key=null` の後方互換は維持する。
