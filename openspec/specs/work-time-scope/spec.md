# work-time-scope Specification

## Purpose
日の「総作業時間」の算入スコープを規定する。未グループ（`ungrouped` = `UNGROUPED_KEY`）バケットを総作業時間へ含めるかを設定 `exclude_ungrouped_from_total`（既定 OFF）で制御し、その除外を集計 source（`totalWorkMsForDay`）・range サマリ・パスワードゲート評価・ダッシュボード表示へ一貫して波及させる。除外は読み出し時にのみ適用し、`daily_totals_snapshot` の per-group 生データは不変に保つ。
