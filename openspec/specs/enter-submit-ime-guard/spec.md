# enter-submit-ime-guard Specification

## Purpose

フォーム／エディタにおける Enter キーの挙動を統一する。IME 変換確定 Enter を全ハンドラでガードして誤動作を防ぎ、単一行フォームでは Enter で主要アクションを実行、複数行エディタでは素の Enter=改行を維持し Ctrl/Cmd+Enter で送信する。あわせて disabled ボタン中の二重送信防止と number 入力の NaN 検証を規定する。
