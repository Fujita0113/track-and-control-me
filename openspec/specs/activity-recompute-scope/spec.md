# activity-recompute-scope Specification

## Purpose
`recompute` が特定の対象日（`onlyDays`）のみを再計算する際に、`raw_sample` の読み込み範囲を対象レンジ直前のバッファ以降に絞り込み、確定済み過去履歴の全件読み直しを避けつつ、絞り込みの有無によらず集計結果が一致することを保証する。
