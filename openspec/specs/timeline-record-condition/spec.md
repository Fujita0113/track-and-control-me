...
ルール条件は新ターゲット `TIMELINE` を持つ SHALL。`TIMELINE` 条件は、評価対象日 `dayKey` の `activity_log_entry` のうち `entry_type='MANUAL'` かつ `category_key` が条件のラベルに一致するエントリの継続時間（`end_at - start_at`）の合計秒が、条件の `threshold_seconds` 以上のとき達成（met=true）とする SHALL。一致は `category_key` 文字列の完全一致で行い、`AUTO_SESSION` エントリは対象にしない（MUST NOT）。合計は重なりの有無を問わず単純合算とする（手動記録は同一ラベルで重複させない前提）。

#### Scenario: 指定ラベルの手動記録が閾値以上で達成

- **WHEN** ラベル「運動」・閾値30分の `TIMELINE` 条件を評価し、当日「運動」カテゴリの MANUAL 記録が合計35分ある
- **THEN** その条件は met=true

#### Scenario: 閾値未満は未達成

- **WHEN** 同じ条件で当日「運動」の MANUAL 記録が合計20分しかない
