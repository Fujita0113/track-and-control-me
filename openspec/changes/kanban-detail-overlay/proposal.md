...
- `server/static/js/kanban.js`（2巡目・design D8）: `cardEl(t)` の単発クリックによる detail オープンを `CARD_OPEN_DELAY_MS`（300ms）遅延させ、dblclick が overlay 生成前に確定するようにする。オーバーレイ導入で「ダブルクリックでカードタイトルをリネーム」（`kanban-card-quick-actions.spec.ts`/`kanban-rename-reorder-reentrancy.spec.ts`）が構造的に壊れる問題への対処（詳細は design.md D5〜D8）。
